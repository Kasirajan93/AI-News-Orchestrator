...
# ============================
# 📡 FEED CONTROLS (FINAL FIX)
# ============================

st.sidebar.subheader("Feed Controls")
...
# 🔁 Refresh ALL feeds
# ------------------------------
if st.sidebar.button("🔁 Refresh All Feeds (Instant)"):
    st.sidebar.info("Refreshing all feeds…")
    try:
        import fetcher
        total = 0
        for feed_url in feeds:
            st.sidebar.write(f"Fetching: {feed_url}")
            total += fetcher.process_feed(feed_url, max_items)
        st.sidebar.success(f"Done! Saved {total} new articles.")
    except Exception as e:
        st.sidebar.error(f"Feed refresh failed: {e}")

# ------------------------------
# 🔁 Refresh SELECTED feed
# ------------------------------
if st.sidebar.button("🔁 Refresh Selected Feed (Instant)"):
    if feed_choice == "-- All feeds --":
        st.sidebar.warning("Choose a specific feed first.")
    else:
        st.sidebar.info(f"Refreshing: {feed_choice}")
        try:
            import fetcher
            count = fetcher.process_feed(feed_choice, max_items)
            st.sidebar.success(f"Saved {count} new articles.")
        except Exception as e:
            st.sidebar.error(f"Refresh failed: {e}")

# ------------------------------
# 📊 Show recent metrics (optional)
# ------------------------------
with st.sidebar.expander("📊 Recent Metrics (Optional)"):
